...
$$
\sum\limits_{i = 1}^{n} [i \in \mathbb{P}] f(i)
$$

考虑先线性筛出不大于 $\sqrt{n}$ 的所有素数作为 $\mathbb{P}$。记 $minp(x)$ 代表 $x$ 的最小素因子，令：
...
第一部分我们之前已经解决了，就是 $g(n, |\mathbb{P}|) - \sum\limits_{k = 1}^{j - 1} f(P_k)$。
...
$$
\sum\limits_{k = j}^{|\mathbb{P}|}\sum\limits_{{P_k}^{e + 1} \le n} \left[ f({P_k}^e) \cdot s(\left\lfloor \frac{n}{{P_k}^{e}} \right\rfloor, k + 1) + f({P_k}^{e + 1}) \right]
$$
...
$$
\begin{aligned}
s(n, j) & =  g(n, |\mathbb{P}|) - \sum\limits_{k = 1}^{j - 1}f(P_k) \\ 
& + \sum\limits_{k = j}^{|\mathbb{P}|}\sum\limits_{{P_k}^{e + 1} \le n} \left[ f({P_k}^e) \cdot s(\left\lfloor \frac{n}{{P_k}^{e}} \right\rfloor, k + 1) + f({P_k}^{e + 1}) \right]
\end{aligned}
$$
...
首先可以试试洛谷上的模板题：[Luogu P5325: Min25筛](https://www.luogu.org/problem/P5325)。同时附上 [我的代码](https://github.com/codgician/Competitive-Programming/blob/master/Luogu/P5325/min25_sieve.cpp) 以供参考。
...
\begin{aligned}
s(n, j) 
& = \sum\limits_{i = P_j + 1}^{n} \left[ i \in \mathbb{P} \right]f(i) + \sum\limits_{i = P_j + 1}^{n} \left[i \not \in \mathbb{P} \right] f(i) \\
& = g(n, |\mathbb{P}|) - \sum\limits_{k = 1}^{j - 1}f(P_k) \\ 
& + \sum\limits_{k = j}^{|\mathbb{P}|}\sum\limits_{{P_k}^{e + 1} \le n} \left[ f({P_k}^e) \cdot s(\left\lfloor \frac{n}{{P_k}^{e}} \right\rfloor, k + 1) + f({P_k}^{e + 1}) \right]
\end{aligned}
$$

对于第一部分，也就是 $\sum\limits_{i = P_j + 1}^{n} \left[ i \in \mathbb{P} \right]f(i)$，不必对其进行修改。由于我们在筛 $s$ 的时候只会用到 $g(n, |\mathbb{P}|)$，其中只包含素数处的值，因此我们大可将其当作积性函数来筛出 $g$，筛 $g$ 的过程保持和上文一样就行了。
...
$$
\sum\limits_{k = j}^{|\mathbb{P}|}\sum\limits_{{P_k}^{e + 1} \le n} \left[ m \cdot f({P_k}^e) +  s(\left\lfloor \frac{n}{{P_k}^{e}} \right\rfloor, k + 1) + f({P_k}^{e + 1}) \right]
$$
...
这也是我自己 YY 的一种应用，不排除有更优秀的做法。对此我也 YY 了一道题：[春燕的板子题](https://www.luogu.org/problem/U87962)（也没有严格验过题，而且由于洛谷私人题库有总时限限制所以可能有点卡常，这只是一个 Proof of concept）。
...
Min25 筛的思想是否可以应用在非前缀和的地方，比如…… 前缀积？对此我自己 YY 了一道题：[春燕的数列](https://www.luogu.org/problem/U87823)（没有严格验过题，只是一个 Proof of concept）。
...
$$
s(n, j) = \prod\limits_{i = P_j + 1}^{n} \left[ i \in \mathbb{P} \right]d(i) \cdot \prod\limits_{i = P_j + 1}^{n} \left[i \not \in \mathbb{P} \right] d(i) 
$$
