...
由于左陪集和右陪集是相对类似的，因此接下来只讨论左陪集的相关性质。对于右陪集，就请读者自行尝试啦~
...
若把 $H$ 的左陪集代表元系的大小 $|R|$ 称作群 $H$ 对于群 $G$ 的**指数 (index)** 并记作 $[G : H]$，便得到抽象代数里的拉格朗日定理：
...
更进一步，如果 $G$ 中两个不同的置换 $\sigma, \tau$ 作用于 $x$ 后的结果是相同的，可以认为 $\sigma, \tau$ 在仅考虑作用于 $x$ 时是两个等价的置换。由此，$| \text{orb}_G(x) |$ 实际上等价于仅考虑作用于 $x$ 时 $G$ 中本质不同的置换种数。
